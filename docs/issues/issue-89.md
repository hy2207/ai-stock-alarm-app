# [LLM-Q01] [Query] GEMINI_MODEL 환경 변수 교체 검증 — 모델 변경 후 동일 스키마 출력 호환성 확인

- Issue: #89
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/89
- Author: hy2207
- Created: 2026-05-14T12:15:48Z
- Updated: 2026-05-14T12:39:10Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/LLM-Q01-Gemini_model_swap_validation.md`

## :dart: Summary
- 기능명: [LLM-Q01] GEMINI_MODEL 환경 변수 교체 검증
- 목적: GEMINI_MODEL 환경 변수 교체 검증을(를) 통해 시장 데이터와 사용자 컨텍스트를 기반으로 구조화된 추천 카드를 생성한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F9 REQ-FUNC-084
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — LLM-Q01 / LLM Pipeline / 복잡도 L
...
- [ ] SRS의 §F9 REQ-FUNC-084 요구사항과 LLM-Q01 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: LLM-C04
- [ ] Vercel AI SDK 기반 추천 생성 파이프라인의 파일 위치와 공개 인터페이스 결정
- [ ] GEMINI_MODEL 환경 변수 교체 검증 모듈 구현
...
Scenario 1: 정상 처리
- Given: 선행 태스크(LLM-C04)가 완료되어 있음
- When: GEMINI_MODEL 환경 변수 교체 검증 작업을 실행함
- Then: SRS §F9 REQ-FUNC-084 요구사항을 만족하는 산출물이 생성되고 후속 태스크에서 참조할 수 있다.

Scenario 2: 오류 및 경계 조건 처리
- Given: 누락되거나 허용되지 않은 입력, 빈 데이터, 외부 서비스 실패 중 하나가 발생함
- When: GEMINI_MODEL 환경 변수 교체 검증 로직 또는 검증을 실행함
- Then: 사용자에게 불필요한 5xx 오류를 노출하지 않고 명확한 검증 오류, 빈 상태, 또는 No Call/운영 경보 경로로 처리한다.

Scenario 3: 추적 이벤트 및 후속 연동
- Given: GEMINI_MODEL 환경 변수 교체 검증 완료 후 관련 후속 태스크 또는 사용자 행동이 발생함
- When: 데이터 조회, 저장, 이벤트 발행, 화면 전환 중 필요한 연동을 수행함
- Then: 의존 태스크가 같은 계약을 재사용하고 PostHog/Prisma/외부 SaaS 연동이 누락되지 않는다.

## :gear: Technical & Non-Functional Constraints
- 범위: LLM-Q01는 [Query] GEMINI_MODEL 환경 변수 교체 검증 — 모델 변경 후 동일 스키마 출력 호환성 확인에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: LLM-C04 완료 상태를 전제로 한다.
