# [TEST-F4-01] [Test] saveRiskProfile GWT 단위 테스트 — 허용값 저장 성공, 비허용값(`invalid_mode`) Zod 검증 실패 → 기존 값 …

- Issue: #70
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/70
- Author: hy2207
- Created: 2026-05-14T12:13:50Z
- Updated: 2026-05-14T12:37:06Z
- Closed: Not closed
...
- **Task ID:** `TEST-F4-01`
- **Epic:** Test
- **SRS / PRD:** §F4 REQ-FUNC-031~033 AC
- **Complexity:** M
...
## Spec (local)
Source: `tasks/TEST-F4-01-SaveRiskProfile_unit_test.md`

## :dart: Summary
- 기능명: [TEST-F4-01] saveRiskProfile GWT 단위 테스트
- 목적: saveRiskProfile GWT 단위 테스트을(를) 통해 SRS Acceptance Criteria를 회귀 가능한 자동화 테스트로 고정한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F4 REQ-FUNC-031~033 AC
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — TEST-F4-01 / Test / 복잡도 M
...
- [ ] SRS의 §F4 REQ-FUNC-031~033 AC 요구사항과 TEST-F4-01 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: CONF-C01
- [ ] 자동화 테스트 코드 및 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] saveRiskProfile GWT 단위 테스트에 해당하는 Given/When/Then 테스트 케이스 작성
...
- Given: saveRiskProfile GWT 단위 테스트 대상 구현에 의도적으로 잘못된 입력 또는 실패 mock이 주어짐
- When: 자동화 테스트를 실행함
...
- 범위: TEST-F4-01는 [Test] saveRiskProfile GWT 단위 테스트 — 허용값 저장 성공, 비허용값(`invalid_mode`) Zod 검증 실패 → 기존 값 보존, 세션 재진입 시 기본값 복원에 한정하며 unrelated refactor를 포함하지 않는다.
