# [DTO-001] `saveWatchlist()` Server Action 입력 Zod 스키마 정의 — items 배열(최대 3개), ticker/sector 중 1개 이…

- Issue: #40
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/40
- Author: hy2207
- Created: 2026-05-14T12:10:38Z
- Updated: 2026-05-14T12:33:49Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/DTO-001-SaveWatchlist_input_schema.md`

## :dart: Summary
- 기능명: [DTO-001] saveWatchlist() Server Action 입력 Zod 스키마 정의
- 목적: saveWatchlist() Server Action 입력 Zod 스키마 정의을(를) Zod 기반 계약으로 정의하여 서버 입력, API 응답, LLM 출력, 클라이언트 렌더링 간 데이터 무결성을 보장한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §6.1, §6.2.3 제약
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — DTO-001 / Foundation / 복잡도 M
...
- [ ] SRS의 §6.1, §6.2.3 제약 요구사항과 DTO-001 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: DB-004
- [ ] Zod DTO/schema 및 TypeScript 타입의 파일 위치와 공개 인터페이스 결정
- [ ] saveWatchlist() Server Action 입력 Zod 스키마 정의에 필요한 Zod object/enum/refine 규칙 정의
...
- When: saveWatchlist() Server Action 입력 Zod 스키마 정의 Zod schema로 parse함
...
- 범위: DTO-001는 `saveWatchlist()` Server Action 입력 Zod 스키마 정의 — items 배열(최대 3개), ticker/sector 중 1개 이상 필수 검증에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: DB-004 완료 상태를 전제로 한다.
