# [TEST-F9-02] [Test] LLM 호출 실패 시나리오 단위 테스트 — timeout/5xx/rate limit → No Call 반환 + llm_call_failed …

- Issue: #100
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/100
- Author: hy2207
- Created: 2026-05-14T12:20:55Z
- Updated: 2026-05-14T12:40:21Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/TEST-F9-02-LLM_failure_scenario_test.md`

## :dart: Summary
- 기능명: [TEST-F9-02] LLM 호출 실패 시나리오 단위 테스트
- 목적: LLM 호출 실패 시나리오 단위 테스트을(를) 통해 SRS Acceptance Criteria를 회귀 가능한 자동화 테스트로 고정한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F9 REQ-FUNC-083 AC
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — TEST-F9-02 / Test / 복잡도 M
...
- [ ] SRS의 §F9 REQ-FUNC-083 AC 요구사항과 TEST-F9-02 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: LLM-C06
- [ ] 자동화 테스트 코드 및 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] LLM 호출 실패 시나리오 단위 테스트에 해당하는 Given/When/Then 테스트 케이스 작성
...
- Given: LLM 호출 실패 시나리오 단위 테스트 대상 구현에 의도적으로 잘못된 입력 또는 실패 mock이 주어짐
- When: 자동화 테스트를 실행함
...
- 범위: TEST-F9-02는 [Test] LLM 호출 실패 시나리오 단위 테스트 — timeout/5xx/rate limit → No Call 반환 + llm_call_failed 이벤트 + 에러 페이지 미노출 검증에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: LLM-C06 완료 상태를 전제로 한다.
