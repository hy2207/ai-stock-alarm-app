# [INFRA-003] Vercel 환경 변수 설정 — GEMINI_API_KEY, CRON_SECRET, DATABASE_URL, NEXTAUTH_SECRET, POSTHOG…

- Issue: #26
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/26
- Author: hy2207
- Created: 2026-05-14T12:09:11Z
- Updated: 2026-05-14T12:32:20Z
- Closed: Not closed
...
- **Task ID:** `INFRA-003`
- **Epic:** Infra
- **SRS / PRD:** §4.2.3 REQ-NF-022, CON-010
- **Complexity:** L
...
## Spec (local)
Source: `tasks/INFRA-003-Vercel_environment_variables.md`

## :dart: Summary
- 기능명: [INFRA-003] Vercel 환경 변수 설정
- 목적: Vercel 환경 변수 설정을(를) 통해 개발, 배포, 운영에 필요한 기반 환경을 표준화한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §4.2.3 REQ-NF-022, CON-010
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — INFRA-003 / Infra / 복잡도 L
...
- [ ] SRS의 §4.2.3 REQ-NF-022, CON-010 요구사항과 INFRA-003 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: INFRA-002
- [ ] 배포/환경/프로젝트 인프라 구성의 파일 위치와 공개 인터페이스 결정
- [ ] Vercel 환경 변수 설정 구성 절차 작성 및 적용
...
Scenario 1: 정상 처리
- Given: 선행 태스크(INFRA-002)가 완료되어 있음
- When: Vercel 환경 변수 설정 작업을 실행함
- Then: SRS §4.2.3 REQ-NF-022, CON-010 요구사항을 만족하는 산출물이 생성되고 후속 태스크에서 참조할 수 있다.

Scenario 2: 오류 및 경계 조건 처리
- Given: 누락되거나 허용되지 않은 입력, 빈 데이터, 외부 서비스 실패 중 하나가 발생함
- When: Vercel 환경 변수 설정 로직 또는 검증을 실행함
- Then: 사용자에게 불필요한 5xx 오류를 노출하지 않고 명확한 검증 오류, 빈 상태, 또는 No Call/운영 경보 경로로 처리한다.

Scenario 3: 추적 이벤트 및 후속 연동
- Given: Vercel 환경 변수 설정 완료 후 관련 후속 태스크 또는 사용자 행동이 발생함
- When: 데이터 조회, 저장, 이벤트 발행, 화면 전환 중 필요한 연동을 수행함
- Then: 의존 태스크가 같은 계약을 재사용하고 PostHog/Prisma/외부 SaaS 연동이 누락되지 않는다.

## :gear: Technical & Non-Functional Constraints
- 범위: INFRA-003는 Vercel 환경 변수 설정 — GEMINI_API_KEY, CRON_SECRET, DATABASE_URL, NEXTAUTH_SECRET, POSTHOG_KEY, ONESIGNAL_APP_ID 등 암호화 등록에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: INFRA-002 완료 상태를 전제로 한다.
