# [DTO-009] PostHog 이벤트 taxonomy 스키마 정의 — 클라이언트 16종 + 서버 3종 이벤트명 · 속성(properties) 명세 문서 작성

- Issue: #5
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/5
- Author: hy2207
- Created: 2026-05-14T12:07:02Z
- Updated: 2026-05-14T12:30:03Z
- Closed: Not closed
...
- **Task ID:** `DTO-009`
- **Epic:** Foundation
- **SRS / PRD:** §F7 REQ-FUNC-060, REQ-FUNC-061
- **Complexity:** M
...
## Spec (local)
Source: `tasks/DTO-009-PostHog_event_taxonomy_schema.md`

## :dart: Summary
- 기능명: [DTO-009] PostHog 이벤트 taxonomy 스키마 정의
- 목적: PostHog 이벤트 taxonomy 스키마 정의을(를) Zod 기반 계약으로 정의하여 서버 입력, API 응답, LLM 출력, 클라이언트 렌더링 간 데이터 무결성을 보장한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F7 REQ-FUNC-060, REQ-FUNC-061
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — DTO-009 / Foundation / 복잡도 M
...
- [ ] SRS의 §F7 REQ-FUNC-060, REQ-FUNC-061 요구사항과 DTO-009 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: 선행 태스크 없음
- [ ] Zod DTO/schema 및 TypeScript 타입의 파일 위치와 공개 인터페이스 결정
- [ ] PostHog 이벤트 taxonomy 스키마 정의에 필요한 Zod object/enum/refine 규칙 정의
...
- When: PostHog 이벤트 taxonomy 스키마 정의 Zod schema로 parse함
...
- 범위: DTO-009는 PostHog 이벤트 taxonomy 스키마 정의 — 클라이언트 16종 + 서버 3종 이벤트명 · 속성(properties) 명세 문서 작성에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: 독립 실행 가능해야 한다.
