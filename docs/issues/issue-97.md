# [REC-Q04] [Query/UI] No Call 상태 카드 UI 구현 — 데이터 부족/LLM 실패 시 대체 안내 문구 표시, HTTP 5xx 미발생

- Issue: #97
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/97
- Author: hy2207
- Created: 2026-05-14T12:17:19Z
- Updated: 2026-05-14T12:40:02Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/REC-Q04-No_call_card_ui.md`

## :dart: Summary
- 기능명: [REC-Q04] No Call 상태 카드 UI 구현
- 목적: No Call 상태 카드 UI 구현을(를) 통해 사용자가 홈과 상세 화면에서 실행 가능한 추천 카드 정보를 오류 없이 확인하게 한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F2 REQ-FUNC-013
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — REC-Q04 / Recommendation / 복잡도 L
...
- [ ] SRS의 §F2 REQ-FUNC-013 요구사항과 REC-Q04 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: REC-Q03, UX-010
- [ ] 추천 카드 조회/표시/상호작용 기능의 파일 위치와 공개 인터페이스 결정
- [ ] No Call 상태 카드 UI 구현 조회/표시/상호작용 구현
...
Scenario 1: 정상 처리
- Given: 선행 태스크(REC-Q03, UX-010)가 완료되어 있음
- When: No Call 상태 카드 UI 구현 작업을 실행함
- Then: SRS §F2 REQ-FUNC-013 요구사항을 만족하는 산출물이 생성되고 후속 태스크에서 참조할 수 있다.

Scenario 2: 오류 및 경계 조건 처리
- Given: 누락되거나 허용되지 않은 입력, 빈 데이터, 외부 서비스 실패 중 하나가 발생함
- When: No Call 상태 카드 UI 구현 로직 또는 검증을 실행함
- Then: 사용자에게 불필요한 5xx 오류를 노출하지 않고 명확한 검증 오류, 빈 상태, 또는 No Call/운영 경보 경로로 처리한다.

Scenario 3: 추적 이벤트 및 후속 연동
- Given: No Call 상태 카드 UI 구현 완료 후 관련 후속 태스크 또는 사용자 행동이 발생함
- When: 데이터 조회, 저장, 이벤트 발행, 화면 전환 중 필요한 연동을 수행함
- Then: 의존 태스크가 같은 계약을 재사용하고 PostHog/Prisma/외부 SaaS 연동이 누락되지 않는다.

## :gear: Technical & Non-Functional Constraints
- 범위: REC-Q04는 [Query/UI] No Call 상태 카드 UI 구현 — 데이터 부족/LLM 실패 시 대체 안내 문구 표시, HTTP 5xx 미발생에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: REC-Q03, UX-010 완료 상태를 전제로 한다.
