# [DTO-003] `RecommendationCard` 출력 Zod 스키마 정의 — ticker, direction, entryPrice/Range, targetPrice…

- Issue: #42
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/42
- Author: hy2207
- Created: 2026-05-14T12:10:50Z
- Updated: 2026-05-14T12:34:02Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/DTO-003-RecommendationCard_output_schema.md`

## :dart: Summary
- 기능명: [DTO-003] RecommendationCard 출력 Zod 스키마 정의
- 목적: RecommendationCard 출력 Zod 스키마 정의을(를) Zod 기반 계약으로 정의하여 서버 입력, API 응답, LLM 출력, 클라이언트 렌더링 간 데이터 무결성을 보장한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §6.2.4 제약, REQ-FUNC-020
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — DTO-003 / Foundation / 복잡도 H
...
- [ ] SRS의 §6.2.4 제약, REQ-FUNC-020 요구사항과 DTO-003 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: DB-005
- [ ] Zod DTO/schema 및 TypeScript 타입의 파일 위치와 공개 인터페이스 결정
- [ ] RecommendationCard 출력 Zod 스키마 정의에 필요한 Zod object/enum/refine 규칙 정의
...
- When: RecommendationCard 출력 Zod 스키마 정의 Zod schema로 parse함
...
- 범위: DTO-003는 `RecommendationCard` 출력 Zod 스키마 정의 — ticker, direction, entryPrice/Range, targetPrice/Range, holdDays(1~10), confidenceScore, reasonLine(1~160자), status에 한정하며 unrelated refactor를 포함하지 않는다.
...
- Depends on: DB-005
- Blocks: DTO-004, DTO-005, DTO-006, DTO-010, MOCK-001, LLM-C04, TEST-F5-01
