# [DB-007] `PerformanceRecord` 모델 스키마 작성 — recId(FK), ticker, predictedDirection, realizedReturn…

- Issue: #39
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/39
- Author: hy2207
- Created: 2026-05-14T12:10:32Z
- Updated: 2026-05-14T12:33:43Z
- Closed: Not closed
...
- **Task ID:** `DB-007`
- **Epic:** Foundation
- **SRS / PRD:** §6.2.6 PerformanceRecord
- **Complexity:** L
...
## Spec (local)
Source: `tasks/DB-007-PerformanceRecord_model_schema.md`

## :dart: Summary
- 기능명: [DB-007] PerformanceRecord 모델 스키마 작성
- 목적: PerformanceRecord 모델 스키마 작성을(를) Prisma 기반 SSOT 데이터 모델로 정의하여 후속 API, Server Action, LLM 파이프라인의 저장 구조를 안정화한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §6.2.6 PerformanceRecord
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — DB-007 / Foundation / 복잡도 L
...
- [ ] SRS의 §6.2.6 PerformanceRecord 요구사항과 DB-007 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: DB-005
- [ ] Prisma schema 및 마이그레이션 대상 모델의 파일 위치와 공개 인터페이스 결정
- [ ] Prisma 모델/관계/인덱스를 PerformanceRecord 모델 스키마 작성 요구사항에 맞게 작성
...
- Then: PerformanceRecord 모델 스키마 작성 관련 모델, 필드, 관계, 인덱스가 오류 없이 검증된다.

Scenario 2: 관계 기반 조회 가능
- Given: PerformanceRecord 모델 스키마 작성와 연결된 상위/하위 엔터티 fixture가 존재함
...
- 범위: DB-007는 `PerformanceRecord` 모델 스키마 작성 — recId(FK), ticker, predictedDirection, realizedReturn, hitFlag, evaluationWindowDays, evaluatedAt에 한정하며 unrelated refactor를 포함하지 않는다.
...
- Depends on: DB-005
- Blocks: DTO-006, MOCK-002, REC-Q02, TRUST-Q01, ARC-Q01
