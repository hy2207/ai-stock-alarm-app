# [DTO-004] `RecommendationCard` LLM Structured Output (streamObject) 입력 JSON Schema 정의 — 3벌 카드(a…

- Issue: #48
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/48
- Author: hy2207
- Created: 2026-05-14T12:11:27Z
- Updated: 2026-05-14T12:34:41Z
- Closed: Not closed
...
- **Task ID:** `DTO-004`
- **Epic:** Foundation
- **SRS / PRD:** §3.1 Google Gemini, REQ-FUNC-082
- **Complexity:** H
...
## Spec (local)
Source: `tasks/DTO-004-RecommendationCard_LLM_json_schema.md`

## :dart: Summary
- 기능명: [DTO-004] RecommendationCard LLM Structured Output (streamObject) 입력 JSON Schema 정의
- 목적: RecommendationCard LLM Structured Output (streamObject) 입력 JSON Schema 정의을(를) Zod 기반 계약으로 정의하여 서버 입력, API 응답, LLM 출력, 클라이언트 렌더링 간 데이터 무결성을 보장한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §3.1 Google Gemini, REQ-FUNC-082
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — DTO-004 / Foundation / 복잡도 H
...
- [ ] SRS의 §3.1 Google Gemini, REQ-FUNC-082 요구사항과 DTO-004 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: DTO-003
- [ ] Zod DTO/schema 및 TypeScript 타입의 파일 위치와 공개 인터페이스 결정
- [ ] RecommendationCard LLM Structured Output (streamObject) 입력 JSON Schema 정의에 필요한 Zod object/enum/refine 규칙 정의
...
- When: RecommendationCard LLM Structured Output (streamObject) 입력 JSON Schema 정의 Zod schema로 parse함
...
- 범위: DTO-004는 `RecommendationCard` LLM Structured Output (streamObject) 입력 JSON Schema 정의 — 3벌 카드(aggressive/balanced/conservative) 응답 구조에 한정하며 unrelated refactor를 포함하지 않는다.
