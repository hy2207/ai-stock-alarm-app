# [TEST-F4-02] [Test] Confidence Score UI 전환 응답 시간 테스트 — 3벌 카드 전환 ≤ 300ms 성능 검증 (클라이언트 타이밍 측정)

- Issue: #99
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/99
- Author: hy2207
- Created: 2026-05-14T12:20:49Z
- Updated: 2026-05-14T12:40:15Z
- Closed: Not closed
- Labels: task, phase/P5-Test
- Assignees: None
- Milestone: None

## Body

## Metadata
- **Task ID:** `TEST-F4-02`
- **Epic:** Test
- **SRS / PRD:** §F4 REQ-FUNC-031, REQ-NF-003
- **Complexity:** M
- **Start Date:** 2026-05-18
- **Target Date:** 2026-05-20
- **AI-accelerated effort:** ~0.6d

## Spec (local)
Source: `tasks/TEST-F4-02-Confidence_toggle_performance_test.md`

## :dart: Summary
- 기능명: [TEST-F4-02] Confidence Score UI 전환 응답 시간 테스트
- 목적: Confidence Score UI 전환 응답 시간 테스트을(를) 통해 SRS Acceptance Criteria를 회귀 가능한 자동화 테스트로 고정한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F4 REQ-FUNC-031, REQ-NF-003
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — TEST-F4-02 / Test / 복잡도 M
- Traceability Matrix: [`/tasks/SRS-v1.md#5-traceability-matrix`](./SRS-v1.md#5-traceability-matrix)
- 제품/비즈니스 배경: [`/tasks/PRD_v1.md`](./PRD_v1.md)

## :white_check_mark: Task Breakdown (실행 계획)
- [ ] SRS의 §F4 REQ-FUNC-031, REQ-NF-003 요구사항과 TEST-F4-02 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: CONF-Q01
- [ ] 자동화 테스트 코드 및 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] Confidence Score UI 전환 응답 시간 테스트에 해당하는 Given/When/Then 테스트 케이스 작성
- [ ] 성공, 실패, 경계값, 빈 상태 fixture 준비
- [ ] 외부 서비스 호출은 mock/stub으로 대체
- [ ] 테스트 실패 메시지가 SRS 요구사항 위반 지점을 직접 설명하도록 작성
- [ ] 정상/예외/빈 상태 입력을 모두 처리하는 검증 로직 또는 테스트 fixture 준비
- [ ] 관련 타입, export, import 경로를 정리하고 후속 태스크에서 재사용 가능하게 구성
- [ ] README 또는 주석이 필요한 운영/제약 사항을 최소한으로 문서화

## :test_tube: Acceptance Criteria (BDD/GWT)
Scenario 1: 테스트가 요구사항 위반을 탐지
- Given: Confidence Score UI 전환 응답 시간 테스트 대상 구현에 의도적으로 잘못된 입력 또는 실패 mock이 주어짐
- When: 자동화 테스트를 실행함
- Then: SRS 요구사항 위반 지점이 명확한 실패 메시지로 보고된다.

Scenario 2: 정상 경로 통과
- Given: SRS Acceptance Criteria를 만족하는 구현과 fixture가 준비됨
- When: 테스트 스위트를 실행함
- Then: Given/When/Then 케이스가 모두 통과한다.

Scenario 3: 외부 의존성 격리
- Given: Gemini, Yahoo Finance/Finnhub, OneSignal, PostHog 등 외부 서비스가 필요함
- When: 테스트를 실행함
- Then: 네트워크 호출 없이 mock/stub으로 결정적인 결과를 검증한다.

## :gear: Technical & Non-Functional Constraints
- 범위: TEST-F4-02는 [Test] Confidence Score UI 전환 응답 시간 테스트 — 3벌 카드 전환 ≤ 300ms 성능 검증 (클라이언트 타이밍 측정)에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: CONF-Q01 완료 상태를 전제로 한다.
- 보안: API 키, OAuth 토큰, 사용자 식별자, 원문 입력은 코드/로그에 평문으로 남기지 않는다.

## :checkered_flag: Definition of Done (DoD)
- [ ] 모든 Acceptance Criteria를 충족하는가?
- [ ] 단위 테스트(Unit Test) 및 통합 테스트(Integration Test)가 추가되었고 통과하는가?
- [ ] SonarQube / Linter 등의 정적 분석 도구 경고가 없는가?
- [ ] API 명세서(Swagger 등)가 최신화되었는가?

## :construction: Dependencies & Blockers
- Depends on: CONF-Q01
- Blocks: None identified in `task-list-v1.md`

---
Refs: `tasks/task-list-v1.md`, `tasks/gantt-from-task-list-v1.md`, `docs/SRS-v1.md`
