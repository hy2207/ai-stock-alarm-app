# [DTO-005] `GET /api/recommendations/today` 응답 DTO 정의 — RecommendationCard 배열(1~3개) 또는 No Call 상…

- Issue: #49
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/49
- Author: hy2207
- Created: 2026-05-14T12:11:33Z
- Updated: 2026-05-14T12:34:48Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/DTO-005-Today_recommendations_response_dto.md`

## :dart: Summary
- 기능명: [DTO-005] GET /api/recommendations/today 응답 DTO 정의
- 목적: GET /api/recommendations/today 응답 DTO 정의을(를) Zod 기반 계약으로 정의하여 서버 입력, API 응답, LLM 출력, 클라이언트 렌더링 간 데이터 무결성을 보장한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §6.1, REQ-FUNC-010
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — DTO-005 / Foundation / 복잡도 M
...
- [ ] SRS의 §6.1, REQ-FUNC-010 요구사항과 DTO-005 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: DTO-003
- [ ] Zod DTO/schema 및 TypeScript 타입의 파일 위치와 공개 인터페이스 결정
- [ ] GET /api/recommendations/today 응답 DTO 정의에 필요한 Zod object/enum/refine 규칙 정의
...
- When: GET /api/recommendations/today 응답 DTO 정의 Zod schema로 parse함
...
- 범위: DTO-005는 `GET /api/recommendations/today` 응답 DTO 정의 — RecommendationCard 배열(1~3개) 또는 No Call 상태 응답에 한정하며 unrelated refactor를 포함하지 않는다.
