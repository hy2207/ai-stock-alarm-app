# [MOCK-003] 프론트엔드 UI 개발용 사용자(User + RiskProfile + Watchlist) Mock 데이터 생성

- Issue: #43
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/43
- Author: hy2207
- Created: 2026-05-14T12:10:56Z
- Updated: 2026-05-14T12:34:09Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/MOCK-003-User_profile_watchlist_mock_data.md`

## :dart: Summary
- 기능명: [MOCK-003] 프론트엔드 UI 개발용 사용자(User + RiskProfile + Watchlist) Mock 데이터 생성
- 목적: 프론트엔드 UI 개발용 사용자(User + RiskProfile + Watchlist) Mock 데이터 생성을(를) 고정 fixture로 제공하여 백엔드 구현 전에도 UI, 테스트, 스토리북 검증이 가능하게 한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §6.2.1~3
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — MOCK-003 / Foundation / 복잡도 L
- 제품/비즈니스 배경: [`/tasks/PRD_v1.md`](./PRD_v1.md)

## :white_check_mark: Task Breakdown (실행 계획)
- [ ] SRS의 §6.2.1~3 요구사항과 MOCK-003 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: DB-002, DB-003, DB-004
- [ ] UI/테스트용 mock 데이터 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] 프론트엔드 UI 개발용 사용자(User + RiskProfile + Watchlist) Mock 데이터 생성에 필요한 정상/경계/빈 상태 mock 데이터 정의
...
- Given: UI 개발자가 프론트엔드 UI 개발용 사용자(User + RiskProfile + Watchlist) Mock 데이터 생성 fixture를 import함
...
- 범위: MOCK-003는 프론트엔드 UI 개발용 사용자(User + RiskProfile + Watchlist) Mock 데이터 생성에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: DB-002, DB-003, DB-004 완료 상태를 전제로 한다.
