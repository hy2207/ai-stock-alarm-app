# [MOCK-002] 프론트엔드 UI 개발용 성과 기록(PerformanceRecord) Mock 데이터 생성 — 성공/실패 혼합 30건 + 빈 상태(0건)

- Issue: #54
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/54
- Author: hy2207
- Created: 2026-05-14T12:12:04Z
- Updated: 2026-05-14T12:35:20Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/MOCK-002-Performance_record_mock_data.md`

## :dart: Summary
- 기능명: [MOCK-002] 프론트엔드 UI 개발용 성과 기록(PerformanceRecord) Mock 데이터 생성
- 목적: 프론트엔드 UI 개발용 성과 기록(PerformanceRecord) Mock 데이터 생성을(를) 고정 fixture로 제공하여 백엔드 구현 전에도 UI, 테스트, 스토리북 검증이 가능하게 한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F5, §6.2.6
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — MOCK-002 / Foundation / 복잡도 L
- 제품/비즈니스 배경: [`/tasks/PRD_v1.md`](./PRD_v1.md)

## :white_check_mark: Task Breakdown (실행 계획)
- [ ] SRS의 §F5, §6.2.6 요구사항과 MOCK-002 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: DB-007
- [ ] UI/테스트용 mock 데이터 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] 프론트엔드 UI 개발용 성과 기록(PerformanceRecord) Mock 데이터 생성에 필요한 정상/경계/빈 상태 mock 데이터 정의
...
- Given: UI 개발자가 프론트엔드 UI 개발용 성과 기록(PerformanceRecord) Mock 데이터 생성 fixture를 import함
...
- 범위: MOCK-002는 프론트엔드 UI 개발용 성과 기록(PerformanceRecord) Mock 데이터 생성 — 성공/실패 혼합 30건 + 빈 상태(0건)에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: DB-007 완료 상태를 전제로 한다.
