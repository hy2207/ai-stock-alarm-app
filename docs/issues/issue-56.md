# [PUSH-C01] [Command] OneSignal Web SDK 초기 설정 — 프론트엔드 푸시 구독 연동, 사용자 consentPush 상태 관리

- Issue: #56
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/56
- Author: hy2207
- Created: 2026-05-14T12:12:16Z
- Updated: 2026-05-14T12:35:34Z
- Closed: Not closed
...
- **Task ID:** `PUSH-C01`
- **Epic:** Push
- **SRS / PRD:** §3.1 OneSignal, REQ-FUNC-050
- **Complexity:** M
...
## Spec (local)
Source: `tasks/PUSH-C01-OneSignal_web_sdk_setup.md`

## :dart: Summary
- 기능명: [PUSH-C01] OneSignal Web SDK 초기 설정
- 목적: OneSignal Web SDK 초기 설정을(를) 통해 아침 브리핑 알림, 사용자 동의, 딥링크 랜딩 흐름을 안정적으로 제공한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §3.1 OneSignal, REQ-FUNC-050
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — PUSH-C01 / Push / 복잡도 M
- 제품/비즈니스 배경: [`/tasks/PRD_v1.md`](./PRD_v1.md)

## :white_check_mark: Task Breakdown (실행 계획)
- [ ] SRS의 §3.1 OneSignal, REQ-FUNC-050 요구사항과 PUSH-C01 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: AUTH-C01
- [ ] OneSignal Web SDK, Cron, 딥링크 처리의 파일 위치와 공개 인터페이스 결정
- [ ] OneSignal Web SDK 초기 설정 관련 OneSignal SDK/REST/Cron/딥링크 흐름 구현
...
- When: OneSignal Web SDK 초기 설정 작업을 실행함
- Then: SRS §3.1 OneSignal, REQ-FUNC-050 요구사항을 만족하는 산출물이 생성되고 후속 태스크에서 참조할 수 있다.

Scenario 2: 오류 및 경계 조건 처리
- Given: 누락되거나 허용되지 않은 입력, 빈 데이터, 외부 서비스 실패 중 하나가 발생함
- When: OneSignal Web SDK 초기 설정 로직 또는 검증을 실행함
- Then: 사용자에게 불필요한 5xx 오류를 노출하지 않고 명확한 검증 오류, 빈 상태, 또는 No Call/운영 경보 경로로 처리한다.

Scenario 3: 추적 이벤트 및 후속 연동
- Given: OneSignal Web SDK 초기 설정 완료 후 관련 후속 태스크 또는 사용자 행동이 발생함
- When: 데이터 조회, 저장, 이벤트 발행, 화면 전환 중 필요한 연동을 수행함
- Then: 의존 태스크가 같은 계약을 재사용하고 PostHog/Prisma/외부 SaaS 연동이 누락되지 않는다.

## :gear: Technical & Non-Functional Constraints
- 범위: PUSH-C01는 [Command] OneSignal Web SDK 초기 설정 — 프론트엔드 푸시 구독 연동, 사용자 consentPush 상태 관리에 한정하며 unrelated refactor를 포함하지 않는다.
...
- Depends on: AUTH-C01
- Blocks: AUTH-C04, PUSH-C02, PUSH-Q01
