# [TEST-F9-03] [Test] 카드 검증 실패 재시도 통합 테스트 — 1회 재시도 후 성공/최종 실패 시 No Call + rec_validation_failed 이벤트

- Issue: #101
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/101
- Author: hy2207
- Created: 2026-05-14T12:21:01Z
- Updated: 2026-05-14T12:40:28Z
- Closed: Not closed
...
- **Task ID:** `TEST-F9-03`
- **Epic:** Test
- **SRS / PRD:** §F9 REQ-FUNC-081, REQ-FUNC-021 AC
- **Complexity:** M
...
## Spec (local)
Source: `tasks/TEST-F9-03-Card_validation_retry_test.md`

## :dart: Summary
- 기능명: [TEST-F9-03] 카드 검증 실패 재시도 통합 테스트
- 목적: 카드 검증 실패 재시도 통합 테스트을(를) 통해 SRS Acceptance Criteria를 회귀 가능한 자동화 테스트로 고정한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F9 REQ-FUNC-081, REQ-FUNC-021 AC
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — TEST-F9-03 / Test / 복잡도 M
...
- [ ] SRS의 §F9 REQ-FUNC-081, REQ-FUNC-021 AC 요구사항과 TEST-F9-03 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: LLM-C05
- [ ] 자동화 테스트 코드 및 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] 카드 검증 실패 재시도 통합 테스트에 해당하는 Given/When/Then 테스트 케이스 작성
...
- Given: 카드 검증 실패 재시도 통합 테스트 대상 구현에 의도적으로 잘못된 입력 또는 실패 mock이 주어짐
- When: 자동화 테스트를 실행함
...
- 범위: TEST-F9-03는 [Test] 카드 검증 실패 재시도 통합 테스트 — 1회 재시도 후 성공/최종 실패 시 No Call + rec_validation_failed 이벤트에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: LLM-C05 완료 상태를 전제로 한다.
