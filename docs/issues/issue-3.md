# [DTO-007] `/api/cron/morning-briefing` 응답 DTO 정의 — `{ scheduled, sent, failed }`

- Issue: #3
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/3
- Author: hy2207
- Created: 2026-05-14T11:54:27Z
- Updated: 2026-05-14T12:29:50Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/DTO-007-Morning_briefing_cron_response_dto.md`

## :dart: Summary
- 기능명: [DTO-007] /api/cron/morning-briefing 응답 DTO 정의
- 목적: /api/cron/morning-briefing 응답 DTO 정의을(를) Zod 기반 계약으로 정의하여 서버 입력, API 응답, LLM 출력, 클라이언트 렌더링 간 데이터 무결성을 보장한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §6.1
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — DTO-007 / Foundation / 복잡도 L
...
- [ ] SRS의 §6.1 요구사항과 DTO-007 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: 선행 태스크 없음
- [ ] Zod DTO/schema 및 TypeScript 타입의 파일 위치와 공개 인터페이스 결정
- [ ] /api/cron/morning-briefing 응답 DTO 정의에 필요한 Zod object/enum/refine 규칙 정의
...
- When: /api/cron/morning-briefing 응답 DTO 정의 Zod schema로 parse함
...
- 범위: DTO-007는 `/api/cron/morning-briefing` 응답 DTO 정의 — `{ scheduled, sent, failed }`에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: 독립 실행 가능해야 한다.
