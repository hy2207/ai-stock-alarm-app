# [TEST-F1-01] [Test] saveWatchlist GWT 단위 테스트 — 1개 선택 시 저장 성공, 3개 초과 시 Zod 검증 오류, ticker/sector 모두 …

- Issue: #68
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/68
- Author: hy2207
- Created: 2026-05-14T12:13:37Z
...
- **Task ID:** `TEST-F1-01`
- **Epic:** Test
- **SRS / PRD:** §F1 REQ-FUNC-001~003 AC
- **Complexity:** M
...
## Spec (local)
Source: `tasks/TEST-F1-01-SaveWatchlist_unit_test.md`

## :dart: Summary
- 기능명: [TEST-F1-01] saveWatchlist GWT 단위 테스트
- 목적: saveWatchlist GWT 단위 테스트을(를) 통해 SRS Acceptance Criteria를 회귀 가능한 자동화 테스트로 고정한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F1 REQ-FUNC-001~003 AC
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — TEST-F1-01 / Test / 복잡도 M
...
- [ ] SRS의 §F1 REQ-FUNC-001~003 AC 요구사항과 TEST-F1-01 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: ONB-C01
- [ ] 자동화 테스트 코드 및 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] saveWatchlist GWT 단위 테스트에 해당하는 Given/When/Then 테스트 케이스 작성
...
- Given: saveWatchlist GWT 단위 테스트 대상 구현에 의도적으로 잘못된 입력 또는 실패 mock이 주어짐
- When: 자동화 테스트를 실행함
...
- 범위: TEST-F1-01는 [Test] saveWatchlist GWT 단위 테스트 — 1개 선택 시 저장 성공, 3개 초과 시 Zod 검증 오류, ticker/sector 모두 null 시 오류에 한정하며 unrelated refactor를 포함하지 않는다.
