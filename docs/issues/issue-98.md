# [TEST-F3-01] [Test] 가격/보유기간 Zod 검증 단위 테스트 — entryPrice 또는 Range 필수, holdDays 1~10 정수, 가격 ≤ 0 차단 + …

- Issue: #98
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/98
- Author: hy2207
- Created: 2026-05-14T12:20:43Z
- Updated: 2026-05-14T12:40:08Z
- Closed: Not closed
...
- **Task ID:** `TEST-F3-01`
- **Epic:** Test
- **SRS / PRD:** §F3 REQ-FUNC-020, REQ-FUNC-021 AC
- **Complexity:** M
...
## Spec (local)
Source: `tasks/TEST-F3-01-Price_holdDays_schema_test.md`

## :dart: Summary
- 기능명: [TEST-F3-01] 가격/보유기간 Zod 검증 단위 테스트
- 목적: 가격/보유기간 Zod 검증 단위 테스트을(를) 통해 SRS Acceptance Criteria를 회귀 가능한 자동화 테스트로 고정한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F3 REQ-FUNC-020, REQ-FUNC-021 AC
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — TEST-F3-01 / Test / 복잡도 M
...
- [ ] SRS의 §F3 REQ-FUNC-020, REQ-FUNC-021 AC 요구사항과 TEST-F3-01 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: DTO-010, LLM-C05
- [ ] 자동화 테스트 코드 및 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] 가격/보유기간 Zod 검증 단위 테스트에 해당하는 Given/When/Then 테스트 케이스 작성
...
- Given: 가격/보유기간 Zod 검증 단위 테스트 대상 구현에 의도적으로 잘못된 입력 또는 실패 mock이 주어짐
- When: 자동화 테스트를 실행함
...
- 범위: TEST-F3-01는 [Test] 가격/보유기간 Zod 검증 단위 테스트 — entryPrice 또는 Range 필수, holdDays 1~10 정수, 가격 ≤ 0 차단 + rec_validation_failed 이벤트 발행 검증에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: DTO-010, LLM-C05 완료 상태를 전제로 한다.
