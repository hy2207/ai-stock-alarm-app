...
| #72 | UX-007 | close-ready-branch | `/` App Router 표면에 방향/가격/보유기간/confidence/reasonLine/면책/No Call 표시 연결 |
| #73 | ARC-Q02 | close-ready-branch | `/archive` App Router 화면에서 성공/실패 포함 성과 이력 렌더링 |
...
| #80 | REC-Q03 | close-ready-branch | `RecommendationCardLink`가 실제 RSC 홈 카드로 ticker/direction/price/hold/confidence/reasonLine 렌더링 |
| #82 | UX-008 | close-ready-branch | `/recommendations/[recId]` App Router 상세가 chartless main fold, 근거, 성과, 유사 패턴 진입 제공 |
| #83 | UX-009 | close-ready-local | V2 confidence toggle 및 이벤트 보강 |
| #84 | UX-013 | close-ready-branch | push landing query(`from=push`, `utm_source=onesignal`) 이벤트와 fallback 문서화. OneSignal 실제 발송 증적은 #75/#107에서 별도 확인 |
...
| #97 | REC-Q04 | close-ready-branch | `/` App Router가 인증/데이터 없음 상태를 No Call 안내로 렌더링하고 5xx 없이 처리 |
...
| #104 | TRUST-Q02 | close-ready-branch | `/recommendations/[recId]`, `/archive`에서 성공률/실패/수익률/빈 상태 성과 카드 렌더링 |
