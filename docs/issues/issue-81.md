# [TEST-F10-02] [Test] 사용자 탈퇴 GWT 통합 테스트 — Prisma 데이터 삭제/익명화 검증, PostHog/OneSignal 비식별 처리 호출 검증

- Issue: #81
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/81
- Author: hy2207
- Created: 2026-05-14T12:14:57Z
- Updated: 2026-05-14T12:38:18Z
- Closed: Not closed
...
## Spec (local)
Source: `tasks/TEST-F10-02-User_deletion_integration_test.md`

## :dart: Summary
- 기능명: [TEST-F10-02] 사용자 탈퇴 GWT 통합 테스트
- 목적: 사용자 탈퇴 GWT 통합 테스트을(를) 통해 SRS Acceptance Criteria를 회귀 가능한 자동화 테스트로 고정한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F10 REQ-FUNC-093 AC
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — TEST-F10-02 / Test / 복잡도 M
...
- [ ] SRS의 §F10 REQ-FUNC-093 AC 요구사항과 TEST-F10-02 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: AUTH-C04
- [ ] 자동화 테스트 코드 및 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] 사용자 탈퇴 GWT 통합 테스트에 해당하는 Given/When/Then 테스트 케이스 작성
...
- Given: 사용자 탈퇴 GWT 통합 테스트 대상 구현에 의도적으로 잘못된 입력 또는 실패 mock이 주어짐
- When: 자동화 테스트를 실행함
...
- 범위: TEST-F10-02는 [Test] 사용자 탈퇴 GWT 통합 테스트 — Prisma 데이터 삭제/익명화 검증, PostHog/OneSignal 비식별 처리 호출 검증에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: AUTH-C04 완료 상태를 전제로 한다.
