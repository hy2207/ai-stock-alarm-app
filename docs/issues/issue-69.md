# [TEST-F10-01] [Test] 인증 미들웨어 GWT 단위 테스트 — 미인증 접근 → /login 리다이렉트, 유효 세션 → 통과, 만료 세션 → 갱신 또는 리다이렉트

- Issue: #69
- State: OPEN
- URL: https://github.com/hy2207/ai-stock-alarm-app/issues/69
- Author: hy2207
- Created: 2026-05-14T12:13:44Z
- Updated: 2026-05-14T12:37:00Z
- Closed: Not closed
...
- **Task ID:** `TEST-F10-01`
- **Epic:** Test
- **SRS / PRD:** §F10 REQ-FUNC-091, REQ-FUNC-092 AC
- **Complexity:** M
...
## Spec (local)
Source: `tasks/TEST-F10-01-Auth_middleware_test.md`

## :dart: Summary
- 기능명: [TEST-F10-01] 인증 미들웨어 GWT 단위 테스트
- 목적: 인증 미들웨어 GWT 단위 테스트을(를) 통해 SRS Acceptance Criteria를 회귀 가능한 자동화 테스트로 고정한다.

## :link: References (Spec & Context)
> :bulb: AI Agent & Dev Note: 작업 시작 전 아래 문서를 반드시 먼저 Read/Evaluate 할 것.
- SRS 문서: [`/tasks/SRS-v1.md`](./SRS-v1.md) — §F10 REQ-FUNC-091, REQ-FUNC-092 AC
- 태스크 목록: [`/tasks/task-list-v1.md`](./task-list-v1.md) — TEST-F10-01 / Test / 복잡도 M
...
- [ ] SRS의 §F10 REQ-FUNC-091, REQ-FUNC-092 AC 요구사항과 TEST-F10-01 태스크 범위를 재확인
- [ ] 선행 태스크 상태 확인: AUTH-C02, AUTH-C03
- [ ] 자동화 테스트 코드 및 fixture의 파일 위치와 공개 인터페이스 결정
- [ ] 인증 미들웨어 GWT 단위 테스트에 해당하는 Given/When/Then 테스트 케이스 작성
...
- Given: 인증 미들웨어 GWT 단위 테스트 대상 구현에 의도적으로 잘못된 입력 또는 실패 mock이 주어짐
- When: 자동화 테스트를 실행함
...
- 범위: TEST-F10-01는 [Test] 인증 미들웨어 GWT 단위 테스트 — 미인증 접근 → /login 리다이렉트, 유효 세션 → 통과, 만료 세션 → 갱신 또는 리다이렉트에 한정하며 unrelated refactor를 포함하지 않는다.
- 의존성: AUTH-C02, AUTH-C03 완료 상태를 전제로 한다.
