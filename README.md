...
| 구성 요소 | 방법 | 데이터 범위 |
|---|---|---|
| 예상가 (점 예측) | **Holt 이중지수평활**(α=0.5, β=0.3) + **선형회귀** 추세 외삽의 앙상블 평균 | 최근 **21거래일** (현재 국면 추적) |
| ±1σ 예상 범위 | 일별 로그수익률의 **EWMA 변동성**(λ=0.94, RiskMetrics) × √(보유일수) | 최대 **63거래일** (안정적 밴드) |
...
- 보조 지표: 추세 기울기(%/일), 추세 신뢰도(회귀 R²), 일 변동성(%)
...
학습시켜 1일 예측을 수행하고 실제 종가와 비교합니다 (미래 정보 미사용).
