...
| 구성 요소 | 방법 | 데이터 범위 |
|---|---|---|
| 예상가 (point forecast) | **Damped-Trend Holt Exponential Smoothing** (α=0.5, β=0.3, φ=0.85) + **Linear Regression**(동일 damped 외삽) ensemble → **random walk anchor로 shrinkage** (trend weight 0.3) | 최근 **21거래일** |
...
- **Damped Trend (Gardner-McKenzie, φ=0.85)**: 일별 주가의 drift 추정은 대부분
  노이즈라, 감쇠 없는 추세 외삽이 실측에서 가장 큰 오차 원인이었습니다. Damped trend는
  M-competition 이래 "이기기 어려운 벤치마크"로 검증된 표준 처방입니다
- **Random Walk Shrinkage (w=0.3)**: 실데이터 가중치 스윕에서 추세 100% 반영이
  random walk 기준선보다도 나빴습니다. 예측을 전일 종가 쪽으로 수축시키되 추세 신호를
  30% 보존합니다
- **Conformal Prediction**: 분포 가정 없이 "이 종목의 최근 실제 예측 오차"의 분위수로
  범위 폭을 보정 — fat tail·국면 전환이 자동 반영되고 목표 커버리지(80%)가 구조적으로
  맞춰집니다
- **Yang-Zhang 변동성**: OHLC 4개 값을 모두 사용해 close-to-close 대비 ~8배
  표본 효율 — 20~60일 소표본 제약에 정확히 맞는 range-based 추정기입니다
...
> 실측 검증 (M7 저장 데이터, walk-forward): 범위 적중률 **31% → 85%**,
> 평균 절대 오차 **3.52% → 2.38%** (random walk 하한 2.23%), 방향 편향 **+2.9% → +1.3%**
