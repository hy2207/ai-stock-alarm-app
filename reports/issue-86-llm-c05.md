# Issue #86 - LLM-C05 Validation Retry

## Summary

- Added validation-only retry behavior to `generateRecommendationCards()`.
- On first Zod structured output validation failure, the generator calls `streamObject()` one more time.
- If the retry succeeds, the retry result is returned and no validation failure event is emitted.
- If both attempts fail validation, the function returns No Call and emits `rec_validation_failed` via `captureServerEvent()`.
- Stream/model failures still return No Call without adding broader failure handling, leaving provider/API failure behavior to LLM-C06.

## Verification

- `pnpm test src/lib/llm/__tests__/generateRecommendationCards.test.ts src/lib/analytics/__tests__/serverCapture.test.ts` - passed, 2 files / 14 tests.
- `env DATABASE_URL=file:./test.db pnpm prisma db push` - passed.
- `env DATABASE_URL=file:./test.db pnpm test` - passed, 35 files / 292 tests.
- `pnpm typecheck` - fails on existing baseline type errors outside this issue scope, including `StockAlarmBrand.tsx`, `HomePage.tsx`, `RecommendationDetailPage.tsx`, auth tests/options, OneSignal typing, and ticker history test casting.
- `pnpm lint` - fails because `next lint` opens the initial ESLint configuration prompt.
- `pnpm build` - compiles successfully, then fails during type validation at the existing `StockAlarmBrand.tsx` `StaticImageData` vs `string` error.

## Scope Notes

- This change extends the existing LLM PR branch rather than creating a new branch.
- Tests mock `streamObject()` and `captureServerEvent()`, so no Gemini or PostHog API key is required.
- No persistence logic was added; generated card storage remains for LLM-C07.
- No `/api/events` endpoint was added.
