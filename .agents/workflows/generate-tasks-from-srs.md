...
### Description
- SRS 참조: `docs/SRS-v1.md` (REQ-FUNC-xxx / REQ-NF-xxx)
- 시퀀스: `docs/SRS-v1.md` §3.4
...
| 4 | 인터페이스(API·Server Action) 기준 작업 단위를 상세 구현 단위로 분해 | Next.js Route Handler / Server Action / RSC 로더 / Zod |
...
- SRS에 정의된 **`/api/...` Route Handler**, **Server Action**, **RSC Prisma 조회** 각각을 다음으로 분해한다:
    ...
    | 서버 로직 | Prisma 쿼리, 캐시, LLM `streamObject()` |
    | 인증/권한 | NextAuth 세션, `CRON_SECRET`, 미들웨어 |
    | 에러/No Call 처리 | HTTP 5xx 금지, No Call 카드, 서버 PostHog 이벤트 |
    | 클라이언트 연동 | `posthog-js`, 가격 복사/브로커 리다이렉트 |
    | 테스트 | Route/Action 단위·E2E |
...
- 예시) REQ-NF-001 홈 추천 p95 ≤ 800ms (warm)
    ...
    보안 Task:
    - NextAuth·OAuth 스코프 최소화
    - Vercel 환경 변수·Supabase RLS 점검
    
    운영 Task:
    - PostHog 대시보드 + Vercel 로그 알림
    - `/api/admin/health` freshness 알림
    - Gemini/OneSignal 비용 (REQ-NF-030~031)
    ```
    ...
    | 6단계 | 테스트/운영 Task         | `tests/`, PostHog·Vercel 설정 메모 `tasks/` 또는 `docs/` |
